...
internal sealed class RemoteHookSession : IDisposable
{
    ...
    internal RemoteHookSession(
        SuspendedGameProcess game,
        string hookPath,
        nint remoteModuleBase)
    {
        _game = game;
        _hookPath = hookPath;
    ...
    public void Shutdown()
    {
        ...
        {
            throw new InvalidOperationException(
                $"Hook 关闭入口返回 {exitCode}。");
        }
    }

    private uint InvokeExport(string exportName)
    {
        var exportRva = RemoteHookInjector.GetExportRva(
            _hookPath,
            exportName);
        var remoteAddress = checked(
            _remoteModuleBase + exportRva);
...
internal static unsafe class RemoteHookInjector
{
    ...
    public static RemoteHookSession Inject(
        SuspendedGameProcess game,
        string hookPath)
    {
        ...
        var localKernel32 = NativeMethods.GetModuleHandle(
            Kernel32Name);
        if (localKernel32 == 0)
        {
            throw SuspendedGameProcess.NewWin32Exception(
                "读取本机 kernel32.dll 句柄失败");
        }

        var localLoadLibrary = NativeMethods.GetProcAddress(
            localKernel32,
            LoadLibraryName);
        if (localLoadLibrary == 0)
        {
            throw SuspendedGameProcess.NewWin32Exception(
                "解析 LoadLibraryW 地址失败");
        }

        if (!NativeMethods.GetModuleHandleEx(
                NativeMethods.GetModuleHandleExFromAddress
                | NativeMethods
        ...
        {
            throw SuspendedGameProcess.NewWin32Exception(
                "解析 LoadLibraryW 所属模块失败");
        }

        var procedureModuleName = ReadLocalModuleName(
            localProcedureModule);
        var remoteProcedureModule = FindRemoteModuleBase(
            game.ProcessId,
            procedureModuleName,
            TimeSpan.FromSeconds(10));
        var loadLibraryRva = checked(
            localLoadLibrary - localProcedureModule);
        var remoteLoadLibrary = checked(
            remoteProcedureModule + loadLibraryRva);
        ...
        var remoteHook = FindRemoteModuleBase(
            game.ProcessId,
            Path.GetFileName(fullHookPath),
            TimeSpan.FromSeconds(10));
        var session = new RemoteHookSession(
            game,
            fullHookPath,
            remoteHook);
        session.Start();
    ...
    internal static nint GetExportRva(
        string modulePath,
        string exportName)
    {
        var module = NativeMethods.LoadLibraryEx(
            modulePath,
            0,
            NativeMethods.DontResolveDllReferences);
        if (module == 0)
        {
            throw SuspendedGameProcess.NewWin32Exception(
                $"映射 Hook DLL 以解析 {exportName} 失败");
        }

        try
        {
            var export = NativeMethods.GetProcAddress(
                module,
                exportName);
            if (export == 0)
            {
                throw SuspendedGameProcess.NewWin32Exception(
                    $"Hook DLL 不包含导出 {exportName}");
            }
        ...
        string operation)
    {
        ...
        try
        {
            ...
            if (waitResult != NativeMethods.WaitObject0)
            {
                ...
                throw new InvalidOperationException(
                    $"远程操作 {operation} 返回未知等待状态 "
                    + $"0x{waitResult:X8}。");
            }

            if (!NativeMethods.GetExitCodeThread(
                    thread,
                    out var exitCode))
            {
                throw SuspendedGameProcess.NewWin32Exception(
                    $"读取远程操作 {operation} 返回值失败");
            }

            if (exitCode == NativeMethods.ThreadStillActive)
            {
                throw new InvalidOperationException(
                    $"远程操作 {operation} 在等待结束后仍在运行。");
            }
    ...
    private static void LoadRemoteLibrary(
        nint process,
        nint remoteLoadLibrary,
        string libraryPath)
    {
        var encodedPath = Encoding.Unicode.GetBytes(
            libraryPath + "\0");
    ...
    private static nint FindRemoteModuleBase(
        int processId,
        string moduleName,
        TimeSpan timeout)
    {
        ...
        do
        {
            var result = TryFindRemoteModuleBase(
                processId,
                moduleName);
            if (result != 0)
            {
        ...
        throw new InvalidOperationException(
            $"在游戏进程中找不到模块 {moduleName}。");
    }

    private static nint TryFindRemoteModuleBase(
        int processId,
        string moduleName)
    {
        var snapshot = NativeMethods.CreateToolhelp32Snapshot(
            NativeMethods.Th32CsSnapModule
            | NativeMethods.Th32CsSnapModule32,
        ...
        try
        {
            var entry = new NativeMethods.ModuleEntry
            {
                Size = checked(
                    (uint)sizeof(NativeMethods.ModuleEntry))
            };

            if (!NativeMethods.Module32First(
                    snapshot,
                    &entry))
            {
                return 0;
            ...
            while (NativeMethods.Module32Next(
                snapshot,
                &entry));
    ...
    private static string ReadModuleName(
        NativeMethods.ModuleEntry* entry)
    {
    ...
    private static string ReadLocalModuleName(nint module)
    {
        ...
        {
            var length = NativeMethods.GetModuleFileName(
                module,
                destination,
                bufferLength);
            if (length == 0
            ...
            {
                throw SuspendedGameProcess.NewWin32Exception(
                    "读取 LoadLibraryW 所属模块名称失败");
            }

            return Path.GetFileName(
                new string(destination, 0, checked((int)length)));
        }
    }
}
