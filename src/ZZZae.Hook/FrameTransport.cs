...
internal static class FrameTransport
{
    ...
    public static bool TryEnqueuePacket(
        ushort commandId,
        ...
        ReadOnlySpan<byte> body)
    {
        ...
        var messageLength = checked(
            1 + sizeof(ushort) + sizeof(int) + sizeof(int)
    ...
    public static void TrySendError(string error)
    {
        try
        {
            ...
            var message = new byte[
                1 + sizeof(int) + textLength];
            message[0] = ErrorMessage;
            BinaryPrimitives.WriteInt32LittleEndian(
                message.AsSpan(1),
                textLength);
            encoded.AsSpan(0, textLength).CopyTo(
                message.AsSpan(5));
            SendMessage(message);
        }
        catch
