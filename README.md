...
1. 完全退出正在运行的绝区零。
2. 双击 `ZZZae.exe`。
3. 用上下箭头选择“从注册表读取”“使用资源管理器选择 `ZenlessZoneZero.exe`”或“退出 ZZZae”，按 Enter 确认。选择资源管理器方式后，在文件窗口中找到游戏程序；路径无效时，程序会显示具体原因，按 Enter 返回菜单重新选择；选择“退出”则直接正常退出。
...
无参数启动时，程序会显示“注册表读取 / 资源管理器选择 / 退出”选择菜单；注册表不存在或路径失效时，默认选择资源管理器。也可以使用上述 `--game` 参数跳过菜单。ZZZae 不会扫描磁盘或修改注册表。
